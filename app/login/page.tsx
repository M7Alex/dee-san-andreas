      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="glass rounded-2xl p-8 shadow-2xl shadow-black/50 border border-gold-600/10">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              <label className="block text-stone-400 text-xs font-medium uppercase tracking-wider mb-2">Identifiant</label>
            ...
            <div>
              <label className="block text-stone-400 text-xs font-medium uppercase tracking-wider mb-2">Mot de passe</label>
              <div className="relative">
                ...
                <button type="button" onClick={()=>setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-500 hover:text-gold-400">
                  {showPw ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
              </div>
            </div>
            <button type="submit" disabled={loading}
              className="btn-gold w-full py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-70">
