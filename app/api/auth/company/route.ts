...
export async function POST(req: NextRequest) {
  ...
  if (!valid) {
    ...
    await addLog({ userId: company.id, userLabel: company.name, action: 'LOGIN_FAILED', companyId: company.id, companyName: company.name, details: 'PIN incorrect', ip })
    return NextResponse.json({ error: 'PIN incorrect' }, { status: 401 })
  }

  resetAttempts(ip)
  await addLog({ userId: company.id, userLabel: company.name, action: 'COMPANY_ACCESS', companyId: company.id, companyName: company.name, ip })
