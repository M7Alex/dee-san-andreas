...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!admin) {
      ...
      await addLog({ userId: 'unknown', userLabel: username, action: 'LOGIN_FAILED', details: 'Utilisateur introuvable', ip }).catch(() => {})
    ...
    if (!valid) {
      recordFailedAttempt(ip)
      await addLog({ userId: admin.id, userLabel: username, action: 'LOGIN_FAILED', details: 'Mot de passe incorrect', ip }).catch(() => {})
      return NextResponse.json({ error: 'Identifiants incorrects' }, { status: 401 })
    }

    resetAttempts(ip)

    // Mettre à jour lastLogin + s'assurer que permissions existent
    await updateDb((db) => {
      const a = db.admins.find(a => a.id === admin.id)
      if (a) {
        a.lastLogin = new Date().toISOString()
        // Migration : ajouter permissions si absent
        if (!a.permissions) {
          a.permissions = DEFAULT_PERMISSIONS[a.role] || DEFAULT_PERMISSIONS.admin
        }
      }
      return db
    }).catch(() => {}) // Ne pas bloquer le login si la DB write échoue

    await addLog({ userId: admin.id, userLabel: username, action: 'LOGIN_SUCCESS', ip }).catch(() => {})
