...
function CompanyCard({ company }: { company: Company }) {
  ...
  return (
    <a
      ref={cardRef}
      href={`/company/${company.slug}`}
      onClick={(e) => { e.preventDefault(); navigateTo(`/company/${company.slug}`) }}
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gov-900 overflow-x-hidden" style={{ opacity: loaded ? 1 : 0, transition: 'opacity 0.8s ease' }}>
        ...
        {/* ── Top Bar avec scan-line ── */}
        <ScanHeader>
          <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-gold-500/10 border border-gold-500/20 flex items-center justify-center hover:bg-gold-500/15 transition-colors">
                <EagleLogo size={22} className="header-eagle" />
              </div>
              <div className="hidden sm:flex flex-col">
                <span className="font-serif text-gold-400 text-sm font-medium leading-tight">
                  État de San Andreas
                </span>
                <span className="font-mono text-gold-600/60 text-[9px] tracking-[0.2em] uppercase">
                  Dept. Économique & Entrepreneurial
                </span>
              </div>
            </div>
            <div className="flex items-center gap-3">
